...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('🔍 [Verify Token] 디코딩된 정보:', { email, tenantId, source });

    // ✅ Slack에서 온 경우: tenantId로 직접 조회
    if (source === 'slack' && tenantId) {
      console.log(`🔍 [Verify Token] Slack 토큰 감지: ${tenantId}`);

      const tenantDoc = await db.collection('tenants').doc(tenantId).get();

      if (!tenantDoc.exists) {
        console.error(`❌ [Verify Token] 테넌트를 찾을 수 없음: ${tenantId}`);
      ...
      console.log('✅ [Verify Token] 테넌트 정보 조회 완료:', tenant.brandName);
      ...
      try {
        ...
        console.log(`📊 [Verify Token] FAQ 개수: ${faqCount}`);
      } catch (faqError) {
      ...
      return res.status(200).json({
        success: true,
        source: 'slack',
        tenantId: tenantId, // ⭐ 추가: 명시적으로 tenantId 전달
        tenants: [tenantData],
      });
    }
    ...
    console.log(`🔍 [Verify Token] 이메일로 조회: ${email}`);
    ...
    if (tenantsSnapshot.empty) {
      console.warn(`❌ [Verify Token] 테넌트를 찾을 수 없음: ${email}`);
      return res.status(404).json({
